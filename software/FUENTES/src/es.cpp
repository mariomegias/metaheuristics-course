...
ES::ES(const string & name, const Data * training, unsigned max_evaluations)
...
unsigned ES::do_search(vector<double> & act_sol, double & fitness_act_sol, unsigned max_neighbors, unsigned eval_limit)
{
    ...
    if (FIN_TEMP < ini_temp)
    {
        ...
        do {
            ...
            while ((n_neighbors < max_neighbors) && (n_successes < max_successes) && (n_evaluations < eval_limit))
            {
